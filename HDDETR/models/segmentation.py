...
import HDDETR.util.box_ops as box_ops
from HDDETR.util.misc import NestedTensor, interpolate, nested_tensor_from_tensor_list
...
def sigmoid_focal_loss(
    inputs, targets, num_boxes, alpha: float = 0.25, gamma: float = 2, mask=None
):
    """
    ...
    if mask is None:
        ...
        return ((loss*mask).sum(1) / (mask.sum(1))).sum() / num_boxes
